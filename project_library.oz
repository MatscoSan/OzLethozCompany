...
in
   thread {WaitNeeded Next}
      ...
      Move=if L andthen R then
              if Keys.Left > Keys.Right then turn(left) else turn(right) end
              elseif L then turn(left)
              elseif R then turn(right)
              else forward end
   in
	 ...
	 case Cells of Cell|Rest then
          if IsHead then % TODO change
            ...
            north then
               ...
               {C tk(create rectangle Cell.x*20-12 Cell.y*20-2 Cell.x*20-6 Cell.y*20+2 tags:@R fill:black)} % Left wing
               {C tk(create rectangle Cell.x*20+6 Cell.y*20-2 Cell.x*20+12 Cell.y*20+2 tags:@R fill:black)} % Right wing
            ...
            [] east then
               ...
               {C tk(create rectangle Cell.x*20 Cell.y*20-6 Cell.x*20+4 Cell.y*20-10 tags:@R fill:black)} % Upper wing, repositioned for eastward flight
               {C tk(create rectangle Cell.x*20 Cell.y*20+6 Cell.x*20+4 Cell.y*20+10 tags:@R fill:black)} % Lower wing, repositioned for eastward flight
            ...
            [] west then
               ...
               {C tk(create rectangle Cell.x*20-4 Cell.y*20-10 Cell.x*20 Cell.y*20-6 tags:@R fill:black)} % Upper wing, repositioned for westward flight
               {C tk(create rectangle Cell.x*20-4 Cell.y*20+10 Cell.x*20 Cell.y*20+6 tags:@R fill:black)} % Lower wing, repositioned for westward flight
